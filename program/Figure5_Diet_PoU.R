...
libraries <- c("tidyr", "dplyr", "ggplot2", "reshape2", "RColorBrewer", 
               "conflicted", "cowplot", #"patchwork", "egg", 
               "readxl", 
...
figure_directory <- here("output", "figures", "Figure5_Diet_PoU", format(Sys.Date(),format = "%y%m%d"))
...
for (ALPHA3 in countries) {
  ...
  filename <- paste0("Fig5_", format(Sys.Date(),format = "%y%m%d"), "_", gsub(" ", "_", ALPHA3), ".png")
  png(
