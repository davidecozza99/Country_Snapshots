...
### Indicator database
data <- read_excel(here("data/guatemala.xlsx")) %>% 
...
product <- read_excel("data/guatemala.xlsx", sheet = "products") %>% 
  rename(Pathway = pathway, product = Product, country = Location)   
...
# for (ALPHA3 in countries) {
  ...
  # Fill the database
  # for ("CurrentTrends" in levels(data_country$pathway)) {
    for (cur_prod_group in levels(as.factor(EAT_data$PROD_GROUP))) {
      EAT_data_fill$kcal_adj[which(EAT_data_fill$PROD_GROUP == cur_prod_group & which(EAT_data_fill$Pathway == "CurrentTrends"))] <- EAT_data$kcal[which(EAT_data$PROD_GROUP == cur_prod_group)] * data_country$kcal_mder[which(data_country$year == 2020 & data_country$pathway == "CurrentTrends")] / kcal_fixed
      EAT_data_fill$maxkcal_adj[which(EAT_data_fill$PROD_GROUP == cur_prod_group & which(EAT_data_fill$Pathway == "CurrentTrends"))] <- EAT_data$maxkcal[which(EAT_data$PROD_GROUP == cur_prod_group)] * data_country$kcal_mder[which(data_country$year == 2020 & data_country$pathway == "CurrentTrends")] / kcal_fixed
      EAT_data_fill$minkcal_adj[which(EAT_data_fill$PROD_GROUP == cur_prod_group & which(EAT_data_fill$Pathway == "CurrentTrends"))] <- EAT_data$minkcal[which(EAT_data$PROD_GROUP == cur_prod_group)] * data_country$kcal_mder[which(data_country$year == 2020 & data_country$pathway == "CurrentTrends")] / kcal_fixed
    }
  # }
  ...
  if(ALPHA3 %in% c("ARG", "AUS", "BRA", "CAN", "CHN", "COL", "ETH", "FIN", "DEU", "IND", 
                   "IDN", "MEX", "NOR", "NPL", "TUR", "DNK", "GRC", "RWA", "SWE", "RUS", "GBR","USA", "Guatemala")){
  ...
  # for (cur_path in levels(as.factor(finaldata$Pathway))) {
    for (cur_prod in levels(as.factor(finaldata$PROD_GROUP))) {
      feas <- finaldata$kcalfeasprod[which(finaldata$PROD_GROUP == cur_prod & finaldata$Pathway == "CurrentTrends")]
      min <- finaldata$minkcal_adj[which(finaldata$PROD_GROUP == cur_prod & finaldata$Pathway == "CurrentTrends")]
      max <- finaldata$maxkcal_adj[which(finaldata$PROD_GROUP == cur_prod & finaldata$Pathway == "CurrentTrends")]
      adv <- finaldata$kcal_adj[which(finaldata$PROD_GROUP == cur_prod & finaldata$Pathway == "CurrentTrends")]
      if (feas <= min & min > 0) {
      ...
      if (new_feas > 90 & cur_prod != "CEREALS") {
        ...
        finaldata <- rbind(finaldata,
                           cbind.data.frame(Pathway = "CurrentTrends",
                                            PROD_GROUP = cur_prod,
                                            kcal_adj = 0,
                                            minkcal_adj = 0,
                                            maxkcal_adj = 0,
      ...
      finaldata$newkcal[which(finaldata$PROD_GROUP == cur_prod & finaldata$Pathway == "CurrentTrends" & finaldata$group == cur_prod)] <- new_feas
    }
  # }
